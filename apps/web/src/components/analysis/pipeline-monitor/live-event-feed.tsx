          ...
          {displayEvents.length === 0 ? (
            ...
            displayEvents.map((event, i) => {
              ...
              return (
                ...
                >
                  {/* 도트 */}
                  <span className={`relative z-10 mt-1.5 h-2.5 w-2.5 rounded-full shrink-0 border-2 border-background ${cfg.dotColor}`} />
                  ...
                  {/* 메시지 */}
                  <span className={`break-all ${event.level === 'error' ? 'text-red-600 dark:text-red-400' : ''}`}>
                    {event.message}
                  </span>
                </motion.div>
              );
