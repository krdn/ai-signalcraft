...
// 변화량 표시 컴포넌트
function DeltaIndicator({ before, after, suffix = '' }: { before: number; after: number; suffix?: string }) {
...
export function CompareView({ baseJobId, compareJobId }: CompareViewProps) {
  ...
  // 총 언급량
  const baseTrend = parseModule(base, 'macro-view')?.dailyMentionTrend as Array<{ count: number }> | undefined;
  const compareTrend = parseModule(compare, 'macro-view')?.dailyMentionTrend as Array<{ count: number }> | undefined;
  ...
  const compareDirection = parseModule(compare, 'macro-view')?.overallDirection as string | undefined;

  // 키워드 비교
  const baseKeywords = (parseModule(base, 'sentiment-framing')?.topKeywords as Array<{ keyword: string }> | undefined)?.map(k => k.keyword) ?? [];
  const compareKeywords = (parseModule(compare, 'sentiment-framing')?.topKeywords as Array<{ keyword: string }> | undefined)?.map(k => k.keyword) ?? [];
  ...
  // 감성 비교 차트 데이터
  const sentimentCompareData = baseSentiment && compareSentiment ? [
    { name: '긍정', base: Math.round(baseSentiment.positive * 100), compare: Math.round(compareSentiment.positive * 100) },
    { name: '부정', base: Math.round(baseSentiment.negative * 100), compare: Math.round(compareSentiment.negative * 100) },
    { name: '중립', base: Math.round(baseSentiment.neutral * 100), compare: Math.round(compareSentiment.neutral * 100) },
  ] : null;
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-lg font-bold font-mono">{compareMentions.toLocaleString()}</span>
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-lg font-bold font-mono">{baseSentiment ? Math.round(baseSentiment.positive * 100) : '—'}%</span>
              <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
              <span className="text-lg font-bold font-mono">{compareSentiment ? Math.round(compareSentiment.positive * 100) : '—'}%</span>
            </div>
            {baseSentiment && compareSentiment && (
              <DeltaIndicator before={baseSentiment.positive * 100} after={compareSentiment.positive * 100} suffix="%" />
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-lg font-bold font-mono">{baseSentiment ? Math.round(baseSentiment.negative * 100) : '—'}%</span>
              <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
              <span className="text-lg font-bold font-mono">{compareSentiment ? Math.round(compareSentiment.negative * 100) : '—'}%</span>
            </div>
            {baseSentiment && compareSentiment && (
              <DeltaIndicator before={baseSentiment.negative * 100} after={compareSentiment.negative * 100} suffix="%" />
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {sentimentCompareData ? (
              <ChartContainer config={compareBarConfig} className="aspect-[2/1] w-full">
                <BarChart data={sentimentCompareData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {newKeywords.slice(0, 10).map((k) => (
                    <span key={k} className="rounded-full bg-emerald-500/10 text-emerald-500 px-2.5 py-0.5 text-xs font-medium">
                      +{k}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {goneKeywords.slice(0, 10).map((k) => (
                    <span key={k} className="rounded-full bg-red-500/10 text-red-500 px-2.5 py-0.5 text-xs font-medium">
                      -{k}
                    </span>
                  ))}
                </div>
