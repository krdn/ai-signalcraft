...
export const MODULES: ModuleGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        help: '여론 참여자를 도메인별 이해관계자 집단으로 분류하여 각 집단의 특성과 움직임을 분석합니다.',
        details: [
          '도메인별 이해관계자 집단 분류 (정치: Core/Opposition/Swing, 기업: 투자자/소비자/임직원, 정책: 지지연합/반대연합/전문가 등)',
    ...
    items: [
      ...
      },
      {
        ...
        details: [
          ...
          '전환 기회: 중립·미결정 집단의 신뢰 확보 가능한 접점 발굴',
      ...
      },
      {
        ...
        details: [
          '타겟 전략: 이해관계자 우선순위 결정, 핵심 집단 유지 vs 중립 집단 전환 비용 비교',
      ...
      },
      {
        ...
        details: [
          ...
          '"[현재 상태] — [핵심 돌파구]" 형식의 한 줄 요약(30~50자) 생성',
  ...
  },
  {
    ...
    items: [
      {
        name: '수용도/지지율 추정',
        nameEn: 'Approval & Acceptance Estimate',
        help: '온라인 여론 데이터를 기반으로 플랫폼별 편향을 보정한 도메인별 수용도·지지율·신뢰도 범위를 추정합니다.',
        details: [
          '도메인별 측정 지표: 정치(지지율), 정책(정책 수용도), 공공기관(시민 신뢰도), 교육(기관 만족도)',
          '플랫폼별 원시 감정 비율 산출 후 편향 보정 적용',
          '단일 수치가 아닌 범위(min~max)로 표현',
          '신뢰도(high ±3%p / medium ±5%p / low ±8%p)에 따른 범위 폭 조정',
        ],
        output: '수용도/지지율/신뢰도 범위(min~max), 신뢰도, 플랫폼별 편향 보정 내역',
        theory: 'Public Opinion Measurement · Gallup, 1935',
      },
      {
        name: '프레임 전쟁',
        nameEn: 'Frame War',
        help: '서로 다른 이해관계자 집단이 사용하는 프레이밍 전략의 세력 역학과 담론 구조를 심층 분석합니다.',
        details: [
          '프레임 3분류: 지배적(dominant) / 위협적(threatening) / 반전 가능(reversible)',
          '집단 간 프레임 세력 역학 분석 (강화/약화/독립 관계)',
      ...
      },
      {
        ...
        help: '리스크 분석과 수용도/신뢰도 데이터를 결합하여 도메인별 3가지 시나리오(확산/통제/역전)를 시뮬레이션합니다.',
        details: [
      ...
      {
        name: '목표 달성 시뮬레이션',
        nameEn: 'Outcome Simulation',
        help: 'Stage 1~4의 모든 분석을 종합하여 도메인별 목표 달성 조건과 최적 전략을 도출합니다.',
        details: [
          '수용도/신뢰도 기반선 + 리스크 감점 + 기회 가점 + 프레임 반영으로 달성 확률 산출',
          '달성 조건(3~7개): 현재 이행 상태(met/partial/unmet) 및 중요도 판단',
          '실패 조건(2~5개): 현재 리스크 수준 및 완화 전략',
          '핵심 전략(3~5개): 달성 확률을 높이기 위한 우선순위 재배치',
        ],
        output: '달성 확률(0~100%), 달성·실패 조건 목록, 핵심 전략, 종합 요약',
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: '평판 지수 측정',
        nameEn: 'Reputation Index (RepTrak)',
        help: 'RepTrak 7차원 모델(Fombrun & van Riel, 2004)로 온라인 여론에서 기업 평판 점수를 측정하고 취약 차원과 개선 방향을 제시합니다.',
        details: [
          'RepTrak 7차원: 제품/서비스·혁신·직장환경·거버넌스·시민의식·리더십·재무',
          '차원별 여론 점수(0~100) 및 추세(개선/유지/악화)',
          '이해관계자 집단별(투자자/소비자/임직원 등) 인식 차이 분석',
      ...
      {
        name: 'SCCT 위기 유형 분류',
        nameEn: 'Crisis Type Classifier (SCCT)',
        help: 'Situational Crisis Communication Theory(Coombs, 2007)로 위기 유형(희생자형/사고형/예방가능형)을 분류하고 Image Repair Theory 기반 최적 대응 전략을 매핑합니다.',
        details: [
          'SCCT 3유형: victim(희생자형) / accidental(사고형) / preventable(예방가능형)',
          '귀속 책임 수준(low/medium/high) 판단 및 Image Repair 5전략 우선순위',
          '골든타임 잔여 시간 평가 (critical 0~24h / high 24~48h / medium 48~72h)',
          '과거 유사 위기 이력 및 성공/실패 사례 참조',
        ],
        output: 'SCCT 위기 유형, 책임 수준, 권고 대응 전략, 골든타임 평가',
        theory: 'SCCT · Coombs, 2007 / Image Repair Theory · Benoit, 1997',
      },
      {
        name: '미디어 프레임 의제 설정력',
        nameEn: 'Media Framing Dominance',
        help: 'Media Framing Theory(Entman, 1993)와 Agenda-Setting Theory(McCombs & Shaw, 1972)를 결합하여 언론 기사 프레임과 댓글 여론의 간극을 측정하고 기업 메시지의 의제 설정력을 평가합니다.',
        details: [
          '언론 지배 프레임 강도(0~100) 및 주요 언론사별 프레임 식별',
          '기사 프레임 vs 온라인 댓글 여론 프레임 간극(frameMismatch) 측정',
          '기업 공식 메시지가 언론에 반영된 정도(companyMessageReflection, 0~100)',
          '의제 주도권 판단: company-led / media-led / public-led / contested',
        ],
        output: '지배 프레임, 언론-여론 간극 목록, 기업 메시지 반영도, 의제 주도권',
        theory: 'Media Framing Theory · Entman, 1993 / Agenda-Setting · McCombs & Shaw, 1972',
      },
      {
        name: 'CSR 공약 진정성 간극',
        nameEn: 'CSR Communication Gap',
        help: 'Organizational Hypocrisy Theory(Brunsson, 1989)를 적용하여 기업이 주장하는 ESG/CSR 공약과 온라인 여론 평가의 진정성 간극을 측정하고 그린워싱 위험을 진단합니다.',
        details: [
          'E·S·G 차원별 기업 공약 vs 온라인 여론 신뢰도(0~100) 비교',
          '그린워싱(Greenwashing) 위험 수준 진단',
          'CSR 이니셔티브별 평판 ROI 측정(-100~100)',
          '전반적 CSR 신뢰도 점수(0~100) 및 개선 권고',
        ],
        output: 'CSR 신뢰도 점수, E·S·G 진정성 간극, 그린워싱 위험 수준, 이니셔티브별 ROI',
        theory: 'Organizational Hypocrisy · Brunsson, 1989',
      },
      {
        name: '평판 회복 시뮬레이션',
        nameEn: 'Reputation Recovery Simulation',
        help: 'RepTrak·SCCT·Social License to Operate 이론을 종합하여 평판 회복 목표 달성 확률과 우선순위별 최적 회복 전략을 도출합니다.',
        details: [
          '평판 회복 목표 달성 확률(0~100%) 및 신뢰도 수준 산출',
          '회복 조건별 현재 충족 상태(met/partial/unmet) 및 중요도 판단',
          '회복 장애 요인 + 이해관계자별 완화 방안 매핑',
          '우선순위 회복 전략: immediate / short-term / long-term 분류',
        ],
        output: '회복 확률, 회복 조건 상태, 장애 요인, 우선순위 전략, SLO 회복 상태',
        theory: 'RepTrak Recovery · Fombrun, 2004 / SCCT · Coombs, 2007 / SLO · Thomson, 2000',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        nameEn: 'Education Outcome Simulation',
        help: '기관 평판·위기·기회 분석을 종합하여 교육기관 신뢰 회복 확률과 최적 평판 관리 전략을 시뮬레이션합니다.',
        details: [
          '평판 회복 확률(0~100%) 및 핵심 전략 우선순위',
          '이해관계자별(재학생·학부모·고용주) 달성 조건 이행 상태(met/partial/unmet)',
          '언론 프레임 중립화·입학 지원자 신뢰 회복·비리 이슈 해소 조건 평가',
          '전략 우선순위 재배치: 단기 위기 대응 vs 장기 평판 구축',
        ],
        output: '평판 회복 확률, 달성·실패 조건 목록, 채널별 전략, 차별화 포지셔닝',
        theory: 'Institutional Reputation Theory · Fombrun, 1996',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        name: '기관 신뢰 회복 시뮬레이션',
        nameEn: 'Public Trust Recovery Simulation',
        help: '시민 신뢰도·위기·기회를 종합하여 공공기관의 신뢰 회복 확률과 최적 소통 전략을 시뮬레이션합니다.',
        details: [
          '신뢰 회복 확률(0~100%) 및 핵심 소통 전략 우선순위',
          '역량 신뢰·가치 신뢰 회복 조건 이행 상태(met/partial/unmet) 평가',
          '지역 미디어 프레임 중립화·시민 참여 구조 개선·행정 성과 공표 조건 분석',
          '전략 우선순위 재배치: 단기 이슈 대응 vs 장기 신뢰 구축',
        ],
        output: '신뢰 회복 확률, 달성·실패 조건 목록, 주민 타겟별 전략, 채널별 소통 방식',
        theory: 'Public Trust Theory · Levi & Stoker, 2000',
      },
    ],
  },
  {
