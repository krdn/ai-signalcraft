...
#### Stage 2/3 — 전략 심화 및 최종 요약 모듈

> **참고**: `final-summary`는 파이프라인에서 Stage 3으로 단독 실행되나, 전략 심화 모듈과 함께 표기.

> **참고**: `context-distillation` 모듈은 LLM 호출 없는 파이프라인 내부 유틸리티로 추천 매트릭스 제외.

| 모듈            | 최고            | 보통              | 최소             |
| --------------- | --------------- | ----------------- | ---------------- |
| `risk-map`      | Claude Opus 4.6 | Claude Sonnet 4.6 | DeepSeek R1      |
| `opportunity`   | Claude Opus 4.6 | Claude Sonnet 4.6 | DeepSeek R1      |
| `strategy`      | Claude Opus 4.6 | Claude Sonnet 4.6 | DeepSeek R1      |
...
| **B: 가성비 최적**       | Stage1=Flash+Flash-Lite, 심화=Haiku, 핵심=Sonnet 4.6          | ✅ 유효 — Flash-Lite이 이미 적용됨 (sentiment-framing, message-impact) | 중간                |
...
| **G: DeepSeek 혼합**     | Stage1=DeepSeek Chat, 추론=DeepSeek Reasoner, 핵심=Sonnet 4.6 | ⚠️ **개선 필요** — DeepSeek V4 전환으로 캐시 히트 시 최대 57% 절감     | **높음**            |
...
> **주의**: DeepSeek API에서 실제 모델 ID(`deepseek-v4` 또는 다른 이름)를 확인 후 적용할 것.

### 3.3 신규 시나리오 제안

#### 시나리오 H: 2026 추론 특화 (신규 권장)

> 예상 비용: ~$0.45/실행 | 추론 정확도 최적화 — Stage 4 시뮬레이션에 o4-mini 적용
...
> 예상 비용: ~$0.08/실행 | `packages/ai-gateway`에 OpenRouter 프로바이더 추가 필요
