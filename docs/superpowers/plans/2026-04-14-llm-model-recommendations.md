# LLM 모델 추천 문서 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 2026년 4월 기준 전체 LLM 시장 조사를 바탕으로 AI SignalCraft 분석 모듈별 LLM 모델 추천 문서(`docs/llm-model-recommendations.md`)를 작성한다.

**Architecture:** 코드 변경 없이 순수 마크다운 문서를 작성한다. Part 1(시장 분류), Part 2(모듈 매트릭스), Part 3(프리셋 업그레이드 제안) 3개 파트로 구성. `.gitignore`에 `.superpowers/` 추가도 포함.
...
| 파일                                | 액션     | 설명                             |
| ----------------------------------- | -------- | -------------------------------- |
| `docs/llm-model-recommendations.md` | **생성** | 메인 산출물 — LLM 모델 추천 문서 |
| `.gitignore`                        | **수정** | `.superpowers/` 항목 추가        |
...
`.gitignore` 파일을 열고 다음 항목이 없으면 추가한다:
...
```bash
git add .gitignore
git commit -m "chore: .superpowers/ 브레인스토밍 세션 디렉토리 gitignore 추가"
```

---

### Task 2: Part 1 — 2026년 LLM 시장 분류 섹션 작성

**Files:**

- Create: `docs/llm-model-recommendations.md`

- [ ] **Step 1: 문서 헤더 및 Part 1 작성**

`docs/llm-model-recommendations.md` 파일을 생성하고 아래 내용을 작성한다:
...
```bash
git add docs/llm-model-recommendations.md
git commit -m "docs: LLM 추천 가이드 — Part 1 시장 분류 작성"
```

---

### Task 3: Part 2 — 분석 모듈별 추천 매트릭스 작성

**Files:**

- Modify: `docs/llm-model-recommendations.md`

- [ ] **Step 1: Part 2 내용 추가**

`docs/llm-model-recommendations.md`에 Part 1 다음으로 아래 내용을 추가한다:
...
```bash
git add docs/llm-model-recommendations.md
git commit -m "docs: LLM 추천 가이드 — Part 2 모듈별 매트릭스 추가"
```

---

### Task 4: Part 3 — 시나리오 프리셋 업그레이드 제안 작성
...
`docs/llm-model-recommendations.md`에 Part 2 다음으로 아래 내용을 추가한다:
...
> 현재 `packages/core/src/analysis/model-config.ts`의 `MODEL_SCENARIO_PRESETS` 기준.

### 3.1 현재 시나리오 평가

| 시나리오                 | 현재 구성 요약                        | 평가                                                  | 업그레이드 우선순위 |
| ------------------------ | ------------------------------------- | ----------------------------------------------------- | ------------------- |
| **A: 최고 품질**         | Stage1=Gemini 2.5 Pro, 전략=Opus 4.6  | ✅ 유효 — 거의 최적                                   | 낮음                |
| **B: 가성비 최적**       | Stage1=Flash, 전략·요약=Sonnet 4.6    | ✅ 유효 — Flash-Lite 도입으로 추가 절감 가능          | 중간                |
| **C: 초저가**            | Stage1=GPT-4.1 Nano, Stage2=Flash     | ✅ 유효 — DeepSeek V4 전환으로 추가 절감 가능         | 중간                |
| **D: Gemini CLI 무료**   | Gemini CLI OAuth 전체                 | ✅ 유효 — 쿼터 주의                                   | 낮음                |
| **E: 완전 무료**         | Gemini CLI 전체                       | ✅ 유효                                               | 낮음                |
| **F: Claude+Gemini CLI** | CLI 혼합                              | ✅ 유효                                               | 낮음                |
| **G: DeepSeek 혼합**     | Stage1=DeepSeek Chat, 전략=Sonnet 4.6 | ⚠️ **개선 필요** — DeepSeek V4 전환으로 43% 추가 절감 | **높음**            |

### 3.2 시나리오 G 즉시 업그레이드 (DeepSeek V4 전환)

현재 G 시나리오의 `deepseek-chat`(V3)을 `deepseek-v4`로 교체 시 예상 비용 절감:

- 캐시 히트: $0.07/MTok → $0.03/MTok (57% 절감)
- 캐시 미스: $0.27/MTok → $0.30/MTok (소폭 증가, 캐시 히트 비율 높으면 전체 절감)

`packages/core/src/analysis/model-config.ts`의 시나리오 G에서:
...
// 변경 후
'macro-view':       { provider: 'deepseek', model: 'deepseek-v4' },
'segmentation':     { provider: 'deepseek', model: 'deepseek-v4' },
'sentiment-framing':{ provider: 'deepseek', model: 'deepseek-v4' },
'message-impact':   { provider: 'deepseek', model: 'deepseek-v4' },
```
````

> **주의**: DeepSeek API에서 `deepseek-v4` 모델명 확인 후 적용 (실제 API 모델 ID는 다를 수 있음).

### 3.3 신규 시나리오 제안

#### 시나리오 H: 2026 추론 특화 (신규 권장)

> 예상 비용: ~$0.45/실행 | 추론 정확도 최적화

| 모듈                | 프로바이더 | 모델              | 선택 이유                 |
| ------------------- | ---------- | ----------------- | ------------------------- |
| `macro-view`        | gemini     | gemini-2.5-flash  | 빠른 구조 파악            |
| `segmentation`      | gemini     | gemini-2.5-flash  | 빠른 분류                 |
| `sentiment-framing` | anthropic  | claude-sonnet-4-6 | 한국어 감정 분석          |
| `message-impact`    | anthropic  | claude-sonnet-4-6 | 메시지 영향 분석          |
| `risk-map`          | anthropic  | claude-sonnet-4-6 | 리스크 맵핑               |
| `opportunity`       | anthropic  | claude-sonnet-4-6 | 기회 분석                 |
| `strategy`          | anthropic  | claude-sonnet-4-6 | 전략 수립                 |
| `final-summary`     | anthropic  | claude-sonnet-4-6 | 최종 요약                 |
| `approval-rating`   | openai     | o4-mini           | 추론 기반 지지율 분석     |
| `frame-war`         | openai     | o4-mini           | 추론 기반 프레임 분석     |
| `crisis-scenario`   | openai     | o4-mini           | 추론 기반 위기 시나리오   |
| `win-simulation`    | openai     | o4-mini           | 추론 기반 승리 시뮬레이션 |

#### 시나리오 I: OpenRouter 초저가 (미래 확장)

> 예상 비용: ~$0.08/실행 | `ai-gateway`에 OpenRouter 프로바이더 추가 필요

| 모듈 그룹    | 프로바이더 | 모델                        |
| ------------ | ---------- | --------------------------- |
| Stage 1 전체 | openrouter | meta-llama/llama-4-maverick |
| Stage 2 전체 | openrouter | qwen/qwen3.6-plus           |
| Stage 4 전체 | gemini     | gemini-2.5-flash            |

---

## 참고 자료

- [Claude API Pricing](https://platform.claude.com/docs/en/about-claude/models/overview)
- [Gemini API Pricing](https://ai.google.dev/gemini-api/docs/pricing)
- [OpenAI API Pricing](https://openai.com/api/pricing)
- [DeepSeek API Pricing](https://api-docs.deepseek.com/quick_start/pricing)
- [xAI Grok Pricing](https://api.x.ai)
...
```bash
git add docs/llm-model-recommendations.md
git commit -m "docs: LLM 추천 가이드 — Part 3 프리셋 업그레이드 제안 추가"
````
...
| 스펙 요구사항                                   | 담당 태스크 | 상태 |
| ----------------------------------------------- | ----------- | ---- |
| Part 1: LLM 시장 분류 (지원/OpenRouter/Bedrock) | Task 2      | ✅   |
| Part 2: 모듈별 최고/보통/최소 매트릭스          | Task 3      | ✅   |
| Part 3: 기존 A~G 프리셋 평가 + 업그레이드 제안  | Task 4      | ✅   |
| 한국어 성능 명시                                | Task 2, 3   | ✅   |
| .gitignore 추가                                 | Task 1      | ✅   |
...
- TBD/TODO 없음 ✅
- 모든 모델명, 가격, 매트릭스가 구체적으로 명시됨 ✅

### 타입 일관성

- 모델명: 스펙과 플랜 전체에서 일관됨 ✅
- `deepseek-v4` 모델 ID 주의사항 명시됨 ✅
