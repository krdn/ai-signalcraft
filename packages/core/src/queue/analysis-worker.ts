...
export function createAnalysisWorker(): Worker {
  return new Worker('analysis', async (job: Job) => {
    ...
    if (job.name === 'run-analysis') {
      ...
      logger.info(`분석 ${isResume ? '재실행' : '시작'}: job=${dbJobId}, keyword=${keyword}${isResume ? `, options=${JSON.stringify(resumeOptions)}` : ''}`);
      ...
      await getDb().update(collectionJobs)
        .set({ status: finalStatus, updatedAt: new Date() })
        .where(eq(collectionJobs.id, dbJobId));

      logger.info(`분석 완료: completed=${result.completedModules.length}, failed=${result.failedModules.length}, status=${finalStatus}`);
      return result;
    }
