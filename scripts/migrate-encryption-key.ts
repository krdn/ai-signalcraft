...
async function main() {
  ...
  for (const row of rows) {
    try {
      ...
      console.log(
        `[${row.id}] ${row.provider_name} / ${row.name}: 복호화 성공 (${plaintext.slice(0, 6)}...)`,
      );
      ...
      await client.query(
        'UPDATE provider_keys SET encrypted_key = $1, updated_at = NOW() WHERE id = $2',
        [reEncrypted, row.id],
      );
      console.log(`  → 재암호화 완료\n`);
    } catch (err) {
      console.error(`[${row.id}] ${row.provider_name} 실패:`, (err as Error).message);
    }
  }
