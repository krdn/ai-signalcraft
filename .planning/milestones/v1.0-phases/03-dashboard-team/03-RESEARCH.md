...
export const accounts = pgTable('accounts', { /* NextAuth 표준 */ });
export const sessions = pgTable('sessions', { /* NextAuth 표준 */ });
export const verificationTokens = pgTable('verification_tokens', { /* NextAuth 표준 */ });
...
export const pipelineRouter = router({
  getStatus: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      // 분석 모듈 진행 상태도 함께 조회
      const moduleStatuses = await db.select().from(analysisResults)
